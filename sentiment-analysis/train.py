...
def train(m, lossFun, optim, scheduler, epochs, train_loader, val_loader, device,
    ...
    logging.info("-"*10 + "Beginning training" + "-"*10)
        ...
        with torch.no_grad():
            ...
            if val_metrics["loss"] < bestLoss:
                ...
                if saveModel == True:
                    ...
                    logging.info("New best value for validation loss: Saved model to bestModel.pt")
            
            # print information about training progress
            if verbose == True:
                logging.info(("Epoch: {} \t Loss (train): {:.3f} (val): {:.3f} \t" +
...
if __name__ == '__main__':
    ...
    model = config.init_obj('model', models, embed_mat.shape[0], embed_mat).to(device)
    config.model['args']["vocab_size"] = embed_mat.shape[0]
